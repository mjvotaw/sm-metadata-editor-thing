from fuzzytrackmatch.base_genre_search import BasicArtistInfo, BasicTrackInfo
from fuzzytrackmatch import BaseGenreSearch, GenreTag
...
class AnimeThemesSearch(BaseGenreSearch[AnimeTheme, Artist]):
    ...
    def _get_genre_tags_from_artist(self, artist: BasicArtistInfo) -> list[GenreTag]:
        return [GenreTag(name="Anime", score=1)]
    ...
    def _find_matching_anime_themes(self, artist: list[str], title: str, result: SearchResult):
        ...
        for anime in result.anime:
            anime_track_info = self._make_track_info_for_anime(
                anime, artist, title)
    ...
    def _make_track_info_for_anime(self, anime: Anime, artists: list[str], title: str):
        ...
        if anime.animethemes is not None:
            best_matching_theme = self._get_best_matching_theme_artists(
                anime.animethemes, artists)

            if best_matching_theme is not None:
                theme_artists = self._get_artists_from_anime_theme(
                    best_matching_theme)
                if theme_artists is not None:
                    source_url = f"https://animethemes.moe/anime/{anime.slug}"
                    track_info = BasicTrackInfo(title=best_matching_name, artists=theme_artists,
                                                source_url=source_url, raw_object=best_matching_theme, match_score=0)
                    return track_info
    ...
    def _make_track_info_for_theme(self, theme: AnimeTheme):
        if theme.anime is not None and theme.song is not None and theme.song.artists is not None:
            ...
            if theme_title is not None and theme_artists is not None:
                track_info = BasicTrackInfo(
                    title=theme_title, artists=theme_artists, source_url=source_url, raw_object=theme, match_score=0)
                return track_info
